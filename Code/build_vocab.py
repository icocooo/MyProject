...
def prepare_smiles_corpus(smiles_file, output_file):
    """准备SMILES语料库"""
    ...
    for smiles in smiles_list:
        # 处理SMILES字符串为2-gram格式
        chars = list(smiles)
        texts.append(' '.join(chars))

    vocab = WordVocab(texts, max_size=1000, min_freq=1)
    vocab.save_vocab(output_file)
    print(f"SMILES词汇表生成完成: {output_file}")
...
def main():
    ...
    prepare_smiles_corpus('smiles_data.txt', '../Vocab/smiles_vocab.pkl')
